...
def repPunct(text):
    """
    Replaces any punctuation mark in string.punctuation with a space
    """
    pstr = string.punctuation
    for ch in pstr:
...
class WordTrigger(Trigger):
    ...
    def isWordIn(self, text):
        '''
        Returns True is whole word is in text, otherwise False.
        It is not case sensitive
        '''
        wlist = []    
        text = text.lower()              # convert to lower case 
        text = repPunct(text)            # replace punctuation marks with ' '
        wlist = text.split(' ')           # split into words
        return self.word in wlist
...
class PhraseTrigger(Trigger):
    ...
    def evaluate (self, story):
        return self.isPhraseIn(story.getTitle()) or \
            self.isPhraseIn(story.getSubject()) or \
